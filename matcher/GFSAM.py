...
class GFSAM:
    ...
    def calc_confidence(self, ref_feats_sem, ref_masks, tar_feats, pred_masks):
        ...
        Args:
            ref_feats_sem: (1, 1369, 1024)
            ref_masks: (1, 1, 1024, 1024)
            tar_feats: (1, 1369, 1024)
            pred_masks: (1, 1024, 1024)

        Returns:
        """
        ref_feats_sem = ref_feats_sem.view(-1, 1024)
        ref_masks = F.interpolate(ref_masks, (37, 37), mode='bilinear', align_corners=False).flatten() # (1369)
        tar_feats = tar_feats.view(-1, 1024)
        pred_masks = F.interpolate(pred_masks.unsqueeze(0), (37, 37), mode='bilinear', align_corners=False).flatten() # (1369)

        ref_feats = ref_feats_sem[ref_masks > 0] # (1369, 1024)
        tar_feats = tar_feats[pred_masks > 0] # (1369, 1024)

        ref_prototype = ref_feats.mean(dim=0) # (1024)
        tar_prototype = tar_feats.mean(dim=0) # (1024)

        ref_prototype = F.normalize(ref_prototype, dim=0, p=2)
        tar_prototype = F.normalize(tar_prototype, dim=0, p=2)

        similarity = torch.matmul(ref_prototype.unsqueeze(0), tar_prototype.unsqueeze(1)).squeeze(0)
